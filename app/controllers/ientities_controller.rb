...
class IentitiesController < ApplicationController
  def entityparams
    permitted_params=params.require(:ientity).permit(:id, :name, :description, :ipv4, :deleted, :updated_by_id)
    permitted_params[:ipv4] = params[:ipv4] if params[:ipv4].present? && iresource.has_ip
    permitted_params[:ipv4] ||= '127.0.0.1'
    permitted_params
  ...
  def ientity_show_list
    if User.current.anonymous? 
      ...
      if params[:res_id].present?
        iresource = IResource.find_by(id: params[:res_id])
        if iresource && iresource.has_entities && IResource.available_for_user(params[:res_id], User.current.id)
          if iresource.has_ip
            ies = iresource.ientities.active.select(['i_entities.id',:ipv4,:name])
          else
            ies = iresource.ientities.active.select(['i_entities.id',:name])
          end
          render json: { ientities: ies, has_ip: iresource.has_ip, has_entities: iresource.has_entities }
        else
          head :forbidden
        end
      else
  ...
  def save_entity
    if ( ( ITicket.check_security_officer(User.current) || IResgranter.is_granter_for_resource(User.current.id,params[:res_id]) || IResowner.is_owner_for_resource(User.current.id,params[:res_id])) && params[:ie_id].present? )
      ...
      iresource = IResource.find_by(id: params[:res_id])
      if iresource
        ie = IEntity.find_by(id: params[:ie_id])
      if ie && ie.iresource_id == iresource.id
        ie.update(entityparams)
        ies = iresource.ientities.active.select(['i_entities.id',:ipv4,:name,:description])
