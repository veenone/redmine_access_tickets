...
class IticketsController < ApplicationController
  ...
  def edit_ticket_table
    ...
    @itickets = []
    if ISetting.active.where(:param => "at_simple_approvement").first.value.to_i == 0 || ((ITicket.check_granting_cf(@issue_id)[0] == 0 || ITicket.check_security_officer(User.current)) && @tracker_id != tr_new_emp_id ) 
